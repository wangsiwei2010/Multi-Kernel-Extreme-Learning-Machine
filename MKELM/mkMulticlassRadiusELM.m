function [gamma,alpha,obj] = mkMulticlassRadiusELM(K,C,Y,qnorm)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
gamma0 = ones(numKer,1)./(radiusp*numKer).^qnorm;
...
while flag
    ...
    gamma = (tmp1.^(1/(qnorm+1)))./(radiusp.^(1/(qnorm+1))*sum((radiusp.^(1/(qnorm+1))).*(tmp1.^(qnorm/(qnorm+1))))^(1/qnorm)); 
    %%% gamma = tmp1./(sqrt(radiusp)*sum(tmp1.*sqrt(radiusp)));
